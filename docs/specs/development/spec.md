...
│   │   │   ├── agent/           #   Agent 相关组件
│   │   │   ├── chat/            #   聊天相关组件
│   │   │   ├── avatar/          #   Avatar 组件（AniAvatar + Live2DAvatar + AvatarFloat）
...
| `components/avatar/AniAvatar.vue` | Ani 专属 Avatar 入口（组合 useAvatarModel + useLipSync + useEmotionDetector + useAniBehavior） |
| `components/avatar/Live2DAvatar.vue` | Live2D 渲染层（PixiJS WebGL + Cubism 4，接收外部参数驱动，通过 setParam/expression/motion 暴露 API） |
| `composables/avatar/useAniBehavior.ts` | 行为状态机（idle/speaking/listening/reacting）+ 自动行为调度（呼吸/眨眼/头部微动/idle动作） |
| `composables/avatar/useLipSync.ts` | 口型同步（Phase 1 音量驱动：噪声门限 + 指数平滑 + 非对称速度） |
| `composables/avatar/useEmotionDetector.ts` | SSE 文本流情绪检测（关键词词典 + LLM 情绪优先级合并 + 30s TTL 衰减） |
| `composables/avatar/useAvatarModel.ts` | 模型加载 + 参数映射（AvatarModelConfig 抽象，支持多模型扩展） |
| `config/avatar/models/haru.ts` | Haru 模型配置（参数 ID、表情索引、动作组） |
